...
Nmc = 5*10^2
nsamples = 10^7
g = erdos_renyi(Nmc, c/Nmc; seed) |> IndexedBiDiGraph
...
@showprogress for samp in 1:nsamples
    onesample!(X, bp_mc)
    for i in 1:Nmc
        autocorrs_mc[i] .+= @views potts2spin.(X[i,:]) .* potts2spin(X[i,end])
        means_mc[i] .+= @views potts2spin.(X[i,:])
    end
    for ed in edges(g)
        j,i = src(ed), dst(ed)
        energy_mc .+= @views potts2spin.(X[i,2:end]) .* potts2spin.(X[j,1:end-1]) .* J[ed|>idx]
    end
end
...
m_mc = mean(means_mc)
energy_mc ./= nsamples * ne(g)

using JLD2
# jldsave("results/article/popdyn_infinite_er_disordered_posneg_beta0,5_dmax$(bonddims[end])_K$(K).jld2"; bs, bs2times, bs2vars)
jldsave("results/article/monte_carlo_infinite_er_disordered_posneg_beta0,5_Nmc$(Nmc)_nsamp$(nsamples).jld2"; m_mc, autocorr_mc, energy_mc)