...
# σ = 1/60
# bonddims = [5, 10, 15]
# # maxiters = [200, 500, 1200]
# maxiters = [200, 500, 3000]

# ϕᵢ = [t == 0 ? [(1-m⁰)/2, (1+m⁰)/2] : ones(2) for t in 0:T]
# ψ_neutral = [ones(2,2) for t in 0:T]

# # prob_degree = truncated(Poisson(c), 1, 1)
# prob_degree = Dirac(1)
...
#     iterate_fourier_popdyn!(μ_pop, popsize, bs, bs2times, bs2vars, prob_degree, prob_J, prob_h, K, β, ϕᵢ, T; maxiter, svd_trunc = TruncBond(d), tol=1e-10, σ)
# end
...
Nmc = 2*10^2
nsamples = 10^4
degs = reduce(vcat, fill(d, round(Int, Nmc*pdf(prob_degree, d))) for d in support(prob_degree))
@assert isgraphical(degs)
Nmc = length(degs)
g = random_configuration_model(Nmc, degs; seed) |> IndexedBiDiGraph
# J = [rand() for _ in edges(g)] .*2 .- 1.0
J = [1.0 for _ in edges(g)]
h = 0.0
ϕᵢ = [t == 0 ? [(1-m⁰)/2, (1+m⁰)/2] : ones(2) for t in 0:T]
...
m_mc = mean(means_mc)
energy_mc ./= nsamples * Nmc

using JLD2
jldsave("results/article/popdyn_infinite_er_disordered_posneg_beta0,5_dmax$(bonddims[end])_K$(K)_T1.jld2"; bs, bs2times, bs2vars)
jldsave("results/article/monte_carlo_infinite_er_disordered_posneg_beta0,5_Nmc$(Nmc)_nsamp$(nsamples)_T1.jld2"; m_mc, autocorr_mc, energy_mc)